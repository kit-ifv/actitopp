...
import edu.kit.ifv.mobitopp.actitopp.IO.CSVDCModelInformationLoader
import edu.kit.ifv.mobitopp.actitopp.IO.CSVDCParameterLoader
import edu.kit.ifv.mobitopp.actitopp.IO.CSVLinRegEstimatesLoader
...
class ModelFileBase private constructor(
    ...
    private fun initWRDSteps(wrdsteps: HashMap<String, Int>) {
        wrdsteps.forEach { (stepID, maxIndex) ->
            (0..maxIndex).forEach {
                modelInformationWRDsteps[stepID, Category(it)] =
                    loadDistributionInformationFromFile("$RESOURCE_PATH${Configuration.parameterset}/${stepID}_KAT_$it.csv")
            }
        }
    }
